...
//유효성 검사
    ...
    if (preg_match("/[\xA1-\xFE\xA1-\xFEa-zA-Z0-9]{2,10}/", $nick)) {
        ...
        $prepare->execute();
        $ID_anony = $prepare->fetchColumn();
        $sql3 = "UPDATE publixher.TBL_USER SET IN_USE='N' WHERE ID=:ID";
    ...
    if ($isnick == 'Y') {
        //익명일때 실명으로 새로 로그인
        $sql = "SELECT ID_USER FROM publixher.TBL_CONNECTOR WHERE ID_ANONY=:ID_ANONY";
        $prepare = $db->prepare($sql);
        $prepare->bindValue(':ID_ANONY', $userID, PDO::PARAM_STR);
        $prepare->execute();
        $userID = $prepare->fetch(PDO::FETCH_ASSOC);
        $sql2 = "SELECT * FROM publixher.TBL_USER WHERE ID=:ID";
        $prepare2 = $db->prepare($sql2);
        $prepare2->bindValue(':ID', $userID['ID_USER'], PDO::PARAM_STR);
        $prepare2->execute();
