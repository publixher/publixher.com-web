...
if ($_GET['profile']) {   //프로필에선 그사람이 쓴거,그사람이 타겟인거 시간순 노출
    ...
    //찾아온 번호들로 컨텐츠 찾아오기
    $sql2 = "SELECT * FROM publixher.TBL_CONTENT WHERE (DEL='N' AND ID=:ID)";
    ...
    $sql = "SELECT * FROM publixher.TBL_CONTENT WHERE (ID_WRITER = :ID_USER0 AND DEL='N' AND ID_TARGET IS NULL)"  //내거찾기
