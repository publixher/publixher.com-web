...
if ($_GET['profile']) {   //프로필에선 그사람이 쓴거,그사람이 타겟인거 시간순 노출
    ...
    $sql="SELECT ID,ID_WRITER,TITLE,EXPOSE,KNOCK,WRITE_DATE,MODIFY_DATE,FOR_SALE,CATEGORY,SUB_CATEGORY,PRICE,PREVIEW,COMMENT,SALE,FOLDER,CHANGED,MORE,TAG FROM publixher.TBL_CONTENT WHERE (DEL='N' AND MATCH(BODY) AGAINST('*".$_GET['body']."*' IN BOOLEAN MODE) AND REPORT<10) ORDER BY WRITE_DATE DESC LIMIT ".$nowpage.",10";
    $prepare = $db->prepare($sql);
    $prepare->execute();
    $result = $prepare->fetchAll(PDO::FETCH_ASSOC);
}else {  //메인화면에서 노출시켜줄 순
