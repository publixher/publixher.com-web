...
//시간값으로 해시를 만든다
        ...
        if (strpos($referer, 'profileConfig.php')) {
            $result = array('status'=>1,'result' => array('file_name' => $name, 'file_profile' => "profile/" . $filepath, 'file_origin' => 'origin/' . $filepath, 'file_height' => $out_height, 'file_width' => $out_width));
        } else {
            $result = array('status'=>1,'result' => array('file_name' => $name, 'file_crop' => "crop_origin/" . $filepath, 'file_origin' => 'origin/' . $filepath, 'file_height' => $out_height, 'file_width' => $out_width));
        }
